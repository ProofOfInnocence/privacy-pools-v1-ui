...
import { Utxo } from '@/services/core/utxo'
import { prove } from '@/services/core/prover'
import { toFixedHex, poseidonHash2, getExtDataHash, shuffle } from '@/services/core/utils'
...
import { proveInclusion } from '@/services/core/poi'
...
export const createTransactionDataThunk = createAsyncThunk("transaction/createTransactionData", async ({ params, keypair }: { params: CreateTransactionParams, keypair: Keypair }, { rejectWithValue }) => {
    try {
        let membershipProof
        ...
        // Added this code to solve redux non-serializable error
        const amountSerialized = amount.toString()
        return { extData, args, amountSerialized, membershipProof }
    } catch (err) {
...
const transactionSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(createTransactionDataThunk.rejected, (state, action) => {
            throw new Error(`Method createTransactionData has error: ${action.payload}`)
        })
    }
});
